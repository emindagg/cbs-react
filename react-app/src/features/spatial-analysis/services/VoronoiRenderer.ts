...
const SOURCE_ID = 'voronoi-source'
const FILL_LAYER_ID = 'voronoi-fill'
const LINE_LAYER_ID = 'voronoi-line'
...
export class VoronoiRenderer {
  ...
  render(points: GeoJSON.FeatureCollection<GeoJSON.Point>, style: SpatialLayerStyle): void {
    ...
    const voronoi = turf.voronoi(points, { bbox: padded })
    if (!voronoi) return

    const validFeatures = voronoi.features.filter(
      (f): f is GeoJSON.Feature<GeoJSON.Polygon> => f !== null && f.geometry !== null,
    )
