...
class AdminApp {
    ...
    checkAccess() {
        ...
        return urlParams.get('admin') !== null || window.location.hash === '#admin';
    }
    ...
    getTotalPhotos() {
        try {
            ...
            // Count photos from markers/drawings
    ...
    getTotalVideos() {
        try {
            ...
            // Count videos from markers/drawings
            if (data.markers && Array.isArray(data.markers)) {
                data.markers.forEach(m => {
                    if (m.media && Array.isArray(m.media)) {
    ...
    renderDashboard() {
        ...
        document.getElementById('suggestion-count').textContent = suggestions;
    ...
    renderAllReports() {
        ...
        tbody.innerHTML = filtered.map((r, i) => `
            <tr data-index="${this.reports.indexOf(r)}">
                ...
                <td>${r.link ? `<a href="${this.escapeHtml(r.link)}" target="_blank" style="color:#3b82f6">Aç</a>` : '-'}</td>
                <td>${this.formatDate(r.timestamp)}</td>
                <td><span class="badge badge--${r.status}">${this.getStatusLabel(r.status)}</span></td>
                <td>
                    ${r.status === 'pending' ? 
    ...
    renderStats() {
        ...
        document.getElementById('stats-total-projects').textContent = projects;
        document.getElementById('stats-registered-teachers').textContent = teachers;
        document.getElementById('stats-registered-students').textContent = students;
        document.getElementById('stats-total-photos').textContent = photos;
        document.getElementById('stats-total-videos').textContent = videos;
